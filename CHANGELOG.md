...
**定位**：校准代码审查的约束来源、完整复审和验收边界，让需求、方案、范围、结果及现有功能影响得到明确回答。

### 审查与验证

* 审查建议只有经负责人采纳或受到独立规则约束才成为审查基线；兼容方案依据真实消费者、混合版本和迁移路径判断，已批准的整体切换不要求永久保留旧格式。
* 用一份五项回答替代重复清单，并区分已批准目标、旧故障证据、实现验证和原任务是否实际解决。
* 修复后完整复审覆盖原始基线至最终候选，检查共享约束及成功、失败路径；复用仍等价的证据，区分实施者自审和实际独立审查。
* 分开判断合并、部署和业务验收，要求阻塞问题说明具体门槛及最小补证，不将无关任务或后置生产验证强加为合并条件。
* 允许用合成数据在内存或临时目录运行独立反例，继续保护被审代码、Git 状态、私有配置和共享服务；临时目录不视为导入副作用隔离。
* 同步 PR 评论五项字段及发布流程的独立复审要求，校准目标文件行号引用。

### 本版本包含的既有主分支改动

* 包含 PR #24 的 `frontend-design` 上游同步，以及 PR #25 的 Office 辅助脚本清理。

### 验证

* Skill 开发验收：仓库测试 266/266、聚焦检查 9/9、离线行为场景 27/27、选中断言 104/104 通过；独立复核发现的问题已修正。
* 两个 Skill 的结构校验、相对链接及差异格式检查通过；完整失败修正过程与未执行范围见评测记录。
* 本轮未执行真实 PR 评论投递、共享服务复现、自动触发路由或跨模型稳定性评测，离线结果不代表这些场景已验收。

### 关联文档

* [代码变更审查校准记录](skills/code-change-review/evals/2026-09-24-review-calibration.md)

---

## [v0.8.0] - 2026-09-21

**定位**：补强现有 `code-change-review` 的需求、方案和实现判断，并支持独立审查后按授权发表评论。

### 审查与评论

* 分别判断需求必要性、方案合理性和实现正确性，追踪界面删减对实际任务的影响，以及搜索的数据范围、计数、分页和 URL 状态。
* 将搜索覆盖完整与规模适用性分开判断；有明确容量上限或离线要求时保留合适的本地过滤，证据不足时不虚构性能故障或规范禁令。
* 与 `review-and-release-pr` 共用评论规范，覆盖讨论纠正、版本变化、去重、正文回读和不确定写入恢复；审查授权不自动扩大为评论、正式批准、修改或合并授权。

### 本版本包含的既有主分支改动

* 澄清工程规则与相关 Skill 的授权继承和验收边界，减少重复确认。
* `handoff` 明确只输出交接材料的边界，以及接收方实施、独立审查和子代理能力要求。

### 验证

* 仓库测试 267/267、两个审查 Skill 的结构校验及差异格式检查通过。
* 新增离线行为场景 8/8、断言 27/27 通过；覆盖来源搜索、图表能力删减和评论交付，独立复核问题已修正。
* 本轮未在真实 PR 上验证新评论流程，未执行自动路由或跨模型稳定性评测；离线结果不代表这些场景已经验收。

### 关联文档

* [审查与评论评测记录](skills/code-change-review/evals/2026-09-21-review-comments.md)

---

## [v0.7.0] - 2026-09-07

**定位**：PR 审查增加影响分级、架构与范围约束判断，主动提醒接口、数据库和既有功能变化，识别没有必要或未经认可的能力扩张。

### ✨ 审查与提醒

* `code-change-review` 按八个维度核对需求范围、能力必要性、架构、接口兼容、数据、安全与资源、既有功能和验证完整性；分别输出影响等级、范围与架构结论、缺陷和合并条件。
* 新增、修改或删除接口、表、字段，以及架构和既有功能变化，均在摘要中给出特别提醒；即使兼容、已获认可或没有发现 Bug，也不省略。
* 以变更前可信基线和独立认可依据判断约束；越界即使没有运行缺陷也阻止合并，高影响但符合约束的变更不自动阻塞，依据不足时明确待决策。
* `review-and-release-pr` 复用同一判断规则，覆盖提前停止、通过和修复流程。有评论授权时发布并回读；同一版本、覆盖范围和结论复用已验证评论，写入失败明确披露。
* 保持原有工具支持范围；不新增依赖、独立评分系统或审批平台。

### 🔄 已合入的上游同步

* 包含 PR #21 对 `docx`、`xlsx`、`human-writing`、`neat-freak` 的同步，以及配套脚本、评测和上游锁文件更新。

### 🧪 验证

* 仓库测试 267/267、两个 Skill 的 `quick_validate.py`、`uv lock --check`、sdist/wheel 构建及 `git diff --check` 通过。
* 离线行为案例 31/31、断言 120/120 通过；其中代码审查 24 个案例、PR 编排 7 个案例。安装目标与被测 Skill 文件一致。
* 两个真实 PR 分别验证提前停止（`STOP`）与带非阻塞问题通过（`PASS`）：评论发布、完整回读及重复复用均通过，每个案例重复投递新增评论 0 条；验证期间均未合并或修改 PR 代码。
* 真实验证不覆盖修复（`FIX`）、版本变化后重新发布、评论失败恢复或自动触发路由，也不代表生产运行验收。

### 📄 关联文档

* [PR 影响与架构约束审查设计](docs/superpowers/specs/2026-09-07-pr-impact-review-design.md)
* [实施、实测与收尾记录](docs/superpowers/plans/2026-09-07-pr-impact-review.md)
