...
- `missing`：创建直接软链；`enabled`：no-op。这样全量 adopt 能一次收敛当前五个入口。
...
7. 记录本次新建的父目录；回滚时只删除仍为空且 inode/identity 与本次创建记录一致的目录，不碰预先存在或已被竞争者写入的目录。

计划指纹由 repo/source identity、源 SHA-256、replace flag、选中 target、动作和目标完整快照规范化计算；快照文件名固定为 `instructions-<fingerprint>.json`。因此同一文件系统状态下，replace preview 与 apply 返回相同 changes、指纹和快照路径。apply 若发现该路径已存在，不覆盖已有 prepared/committed 记录，而是返回 `incomplete-transaction` 或 `snapshot-conflict` 供用户先审查。
