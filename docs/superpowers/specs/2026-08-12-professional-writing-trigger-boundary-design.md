...
合并后的 fresh-agent 基线显示：自主正式复盘和常规进展汇报仍会选择 `professional-writing`，但“继续刚才的 Superpowers 设计流程并写入可评审设计文档”仍同时选择了 `professional-writing`。因此不能把问题归因于正向类别完全丢失；可复现根因是负向规则只有抽象的“不得仅因”，没有把“用户明确要求继续或按原流程产出必需文档”写成直接排除条件。

同时，初版设计明确要求覆盖“用户显式请求 + agent 自主产出文档前”两个入口。当前 description 删除了自主入口和多项自然语言触发词，虽然两个抽样正例仍能正确路由，但合同已弱于原设计，且正文中的自主快速通道只有触发后才能读取。修正必须同时防止未来召回回归。

校准后的规则：

1. 恢复写总结、调研总结、复盘、进展汇报、变更总结、写报告、整理成文档、写成文档给人看，以及 agent 完成工作后主动产出正式文档文件等正向入口。
2. 用户明确要求“继续”或“按”已有设计、开发或治理流程产出该流程必需的 `spec`、`plan` 或 `design doc` 时，不加载 `professional-writing`。
3. 其他流程主导内容判断后，只有用户显式点名、指定组合顺序，或正式文档成为独立交付物时，才加载 `professional-writing`；它不得取代原流程的方案生成职责。
4. 单独从零撰写技术方案、已有文档重写、材料不足但正式文档是主要交付物等正向场景保持不变。

验收增加三个正向样本（自主正式复盘、进展汇报、变更总结），并用同一个 active Superpowers 反例验证 `professional-writing` 不再参与。静态合同锁定描述和矩阵；fresh-agent 对照作为实际路由证据。
